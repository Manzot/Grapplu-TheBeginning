...
public class PlayerController : MonoBehaviour, IDamage
{
    ...
    public void Refresh()
    {
        ...
        if (Input.GetKey(KeyCode.Q))
        {
            rb.gravityScale = 0;
            rb.isKinematic = true;
            rb.linearVelocity = new Vector2(horizontal, Input.GetAxis("Vertical") * 2);
        }
        else
        {
            rb.gravityScale = 3;
    ...
    public void Movement()
    {
        ...
        if (!isSwinging && !RopeSystem.isThrowingHook)
        {
            if (!timeSlow)
            {
                rb.linearVelocity = new Vector2(horizontal * speed   /*timeSlowMo.customFixedUnscaledDeltaTime*/, rb.linearVelocity.y);
            }
            else
            {
                rb.linearVelocity = new Vector2(horizontal * speed   /*timeSlowMo.customFixedUnscaledDeltaTime*/, rb.linearVelocity.y); ;
    ...
    public void Jump()
    {
        if (Grounded() && !RopeSystem.isRopeAttached)
        {
            ...
            if (Input.GetButtonDown("Jump"))
            {
                ...
                {
                    if (!timeSlow)
                        rb.AddForce(new Vector2(rb.linearVelocity.x, jumpForce /** timeSlowMo.customFixedUnscaledDeltaTime*/), ForceMode2D.Impulse);
                    else
                        rb.AddForce(new Vector2(rb.linearVelocity.x, jumpForce /** timeSlowMo.customFixedUnscaledDeltaTime*/), ForceMode2D.Impulse);
    ...
    void GravityCheck()
    {
        if (!Grounded())
        {
            ...
            {
                rb.linearVelocity = new Vector2(rb.linearVelocity.x, maxGravity);
            }
        }
    }
