...
public class ElementalBoss : BossUnit
{
    ...
    public void MoveToPlayer(float speedMultiplier = 1)
    {
      //  anim.SetFloat("xFloat", Mathf.Abs(rb.velocity.x));
        if (!enraged)
            rb.linearVelocity = new Vector2(dirTowardsTarget.normalized.x, dirTowardsTarget.normalized.y / 50) * speed * speedMultiplier* Time.fixedDeltaTime + new Vector2(0, rb.linearVelocity.y);
        else
        {
            rb.linearVelocity = new Vector2(dirTowardsTarget.normalized.x, dirTowardsTarget.normalized.y / 50) * speed * 2f * speedMultiplier * Time.fixedDeltaTime + new Vector2(0, rb.linearVelocity.y);
        }
    }
    ...
    public void RandomMove()
    {
       ...
       // else
        {
                if (moveRight) rb.linearVelocity = new Vector2(1, RANDOM_Y) * speed * speedMult* Time.fixedDeltaTime; // Move Right
                else rb.linearVelocity = new Vector2(-1, RANDOM_Y) * speed * speedMult * Time.fixedDeltaTime; // Move Left
        }
