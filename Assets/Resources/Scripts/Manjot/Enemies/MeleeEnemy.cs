...
public class MeleeEnemy : EnemyUnit
{
    ...
    void FollowPlayer()
    {
        ...
        {
            rb.linearVelocity = (new Vector2(transform.right.x * speed * Time.fixedDeltaTime, rb.linearVelocity.y));
        }
        else
            rb.linearVelocity = new Vector2((target.position.x - transform.position.x), 0).normalized * speed * Time.fixedDeltaTime + new Vector2(0, rb.linearVelocity.y);
    ...
    void FollowWithAstar()
    {
        ...
        if (aStarPath.Count > 0)
        {
            ...
            rb.linearVelocity = newPath * speed * Time.fixedDeltaTime + new Vector2(0, rb.linearVelocity.y);
            //DrawLine(aStarPath);
    ...
    void MoveLeftRight()
    {
        ...
        if (moveRight)
            rb.linearVelocity = new Vector2(1 * (speed) * Time.fixedDeltaTime, rb.linearVelocity.y); // Move Right
        else
            rb.linearVelocity = new Vector2(-1 * (speed) * Time.fixedDeltaTime, rb.linearVelocity.y); // Move Left
    }
    void MoveLeftRight(float speedDivision)
    {
        ...
        if (moveRight)
            rb.linearVelocity = new Vector2(1 * (speed / speedDivision) * Time.fixedDeltaTime, rb.linearVelocity.y); // Move Right
        else
            rb.linearVelocity = new Vector2(-1 * (speed / speedDivision) * Time.fixedDeltaTime, rb.linearVelocity.y); // Move Left
    }
   
    /// Atttacking player and Walking Animations
    void AnimationCaller()
    {
        anim.SetFloat("xFloat", Mathf.Abs(rb.linearVelocity.x));
