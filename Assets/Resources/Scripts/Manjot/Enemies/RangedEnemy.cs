...
public class RangedEnemy : EnemyUnit
{
        ...
        if (!isRuuningFromPlayer)
        {
            ...
            else // if both at same y
            {
                if (distanceToPlayerX > STOPPING_DISTANCE + 4) {
                    ...
                    rb.linearVelocity = new Vector2(dirOppToPlayer.x, rb.linearVelocity.y) * (speed / 1.5f) * Time.fixedDeltaTime;
                }                   
            }
        }
        else
    ...
    void MoveLeftRight()
    {
        ...
        if (moveRight)
            rb.linearVelocity = new Vector2(1 * (speed / 1.5f) * Time.fixedDeltaTime, rb.linearVelocity.y); // Move Right
        else
            rb.linearVelocity = new Vector2(-1 * (speed / 1.5f) * Time.fixedDeltaTime, rb.linearVelocity.y); // Move Left
    }
    void RunAwayFromTarget()
    {
        rb.linearVelocity = new Vector2(dirOppToPlayer.x * speed * Time.fixedDeltaTime, rb.linearVelocity.y);
