   ...
   // public LineRenderer line;
    ...
    public virtual void PhysicsRefresh()
    {
        if(eType != EnemyType.Flying)
        {
            if (!Grounded())
            {
                if (rb.linearVelocity.y < MAX_GRAVITY)
                {
                    rb.linearVelocity = new Vector2(rb.linearVelocity.x, MAX_GRAVITY);
                }
            }
        }
    }
    ...
    public void KnockBack()
    {
        ...
        TimerDelg.Instance.Add(() => { if (rb) { rb.linearVelocity = new Vector2(0, rb.linearVelocity.y); } }, 0.2f);
